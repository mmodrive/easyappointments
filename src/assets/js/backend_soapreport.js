...
(function (exports) {
    ...
    function _loadReports(reports)
    {
        ...
        $report.find('.reports tbody .delete-report').click(function() {
            if (confirm('Are you sure you want to delete this report? This action is not reversible!')) {
                var url = GlobalVariables.baseUrl + '/index.php/backend_api/ajax_delete_soap_report';
                var postData = {
                ...
                var successCallback = function (response) {
                    if (!_handleAjaxExceptions(response)) 
                        return false;

                    // Display success message to the user.
                    $report.find('.modal-message').text(EALang.report_deleted);
                    $report.find('.modal-message').addClass('alert-success').removeClass('alert-danger hidden');
    ...
    function _saveReport()
    {
        ...
        $('#soap_report_form .form-control').each(function () {
            if ($(this).val() != ''){ 
                if(this.id == 'date')
                    report[this.id] = $(this).datepicker('getDate').toString('yyyy-MM-dd');
                // else if(this.id == 'id'){}
                else
                    report[this.id] = $(this).val();
            }
        });

        var url = GlobalVariables.baseUrl + '/index.php/backend_api/ajax_save_soap_report';
        var postData = {
            csrfToken: GlobalVariables.csrfToken,
    ...
    function _handleAjaxExceptions (response) {
        if (!GeneralFunctions.handleAjaxExceptions(response)) {
            $report.find('.modal-message').text(EALang.unexpected_issues_occurred);
            $report.find('.modal-message').addClass('alert-danger').removeClass('hidden');
            return false;
        }
        return true;
    }
    ...
    function _validateSOAPReportForm() {
        ...
        try {
            ...
            $report.find('.modal-message').addClass('alert-danger').text(exc).removeClass('hidden');
            return false;
        }
    }
    ...
    exports.load = function (pet_id) {
        ...
        var url = GlobalVariables.baseUrl + '/index.php/backend_api/ajax_load_soap_reports';
        var postData = {
            csrfToken: GlobalVariables.csrfToken,
            pet_id: pet_id,
        };
