...
<style>
  ...
  .print tr td:nth-child(n+8),
  .print tr th:nth-child(n+8) {
    display: none;
  }
...
<div class="returned-content">
  ...
  ?>
      ...
      $next_new_section = $i + 1 >= count($appointments) || ($appointment['service_name'] . '|' . $appointment['provider_name']) !== ($appointments[$i + 1]['service_name'] . '|' . $appointments[$i + 1]['provider_name']);
      if ($next_new_section) {

        echo '<tr>
          <td colspan="3" class="total_titles">' . 'Total Appointments:' . '</td>
          <td >' . $sec_ttl_app . '</td>
          <td colspan="5"></td>
          </tr>';
        ...
        if ($sec_ttl_cncl > 0) {
          echo '<tr>
            <td colspan="3" class="total_titles">' . 'Total Cancellations:' . '</td>
            <td >' . $sec_ttl_cncl . '</td>
            <td colspan="5"></td>
            </tr>';
          echo '<tr>
            <td colspan="3" class="total_titles">' . '' . '</td>
            <td >' . ($sec_ttl_app + $sec_ttl_cncl) . '</td>
            <td colspan="5"></td>
            </tr>';
        }

        if ($i + 1 >= count($appointments) && $ttl_app != $sec_ttl_app) {
          echo '<tr><td colspan="9"></tr><tr>
            <td colspan="3" class="total_titles">' . 'Grand Total Appointments:' . '</td>
            <td >' . $ttl_app . '</td>
            <td colspan="5"></td>
            </tr>';
          ...
          if ($ttl_cncl > 0) {
            echo '<tr>
              <td colspan="3" class="total_titles">' . 'Grand Total Cancellations:' . '</td>
              <td >' . $ttl_cncl . '</td>
              <td colspan="5"></td>
              </tr>';
            echo '<tr>
              <td colspan="3" class="total_titles">' . '' . '</td>
              <td >' . ($ttl_app + $ttl_cncl) . '</td>
              <td colspan="5"></td>
              </tr>';
          }
        }
