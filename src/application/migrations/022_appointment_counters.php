...
WITH apps AS (
SELECT apps.id AS id_appointment, end_datetime app_datetime, apps.id_services, apps.id_users_customer, servs.disc_num_of_apps_before, servs.disc_timeframe_days, usrsets.disc_qualify,
        NOT ISNULL(discopen.id) AS opening_app,
        IFNULL(DATEDIFF(end_datetime, LAG(end_datetime,1) OVER w),0) AS days_passed,
...
WINDOW w AS (PARTITION BY id_services, id_users_customer ORDER BY end_datetime))
SELECT
id_appointment,
...
ORDER BY id_services, id_users_customer, position;
END
        ');
